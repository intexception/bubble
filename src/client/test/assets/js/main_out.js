(function(wHandle, wjQuery) {
    ...
    function drawChatBoard() {
        ...
        for (var i = 0; i < (len - from); i++) {
            var chatName = new UText(18, chatBoard[i + from].color, "Arial");
            chatName.setValue(chatBoard[i + from].name);
            var width = chatName.getWidth();
            var a = chatName.render();
            ctx.drawImage(a, 15, chatCanvas.height / scaleFactor - 24 * (len - i - from) - 400);

                var chatText = new UText(18, '#666666', "Arial");
            chatText.setValue('' + chatBoard[i + from].message);
            a = chatText.render();
            ctx.drawImage(a, 15 + width * 1.8, chatCanvas.height / scaleFactor - 24 * (len - from - i) - 400);
        }
    }
